...
- **runtime-boundary.md parity refresh:** the boundary contract now reflects the shipped runtime — demo-engine execution of referenced sub-flow documents (namespace-aware matching, depth guard, script-output semantics with `$context.<scriptTask>`), the gateway's AI endpoints (`/ai/chat`, `/ai/agent` with `200`/`400`/`503`/`502` mappings and `aiKind` audit entries), and the provider bridge in the server-side secrets section. (Task 57)
