...
- **Standalone Deployment Bundle Generator:** 1-click generation, preview, copy, and download of container `Dockerfile`, Kubernetes manifest (`deployment.yaml` with ConfigMap, Deployment, Service), `workflow.yaml`, and `README.md` — plus every referenced sub-flow (`subflows/<namespace>/<name>.yaml`) materialized from your workspace documents or the canonical AI contract.
...
**AI orchestration:** the palette **AI** group is live — `LLM call`, `AI agent call`, `Text classifier`, and `Text summarizer` compose valid DSL (sub-flow delegation + catalog-backed providers) and scaffold runnable catalog-backed sub-flows. All components are declared in a data-driven registry (`src/ai/registry.ts`); adding a new AI component = one registry entry. The demo engine executes referenced sub-flow documents from the workspace (falling back to contract-shaped mocks), the Problems panel flags unresolved sub-flow targets (with one-click scaffold), and the deployment bundle ships every referenced sub-flow as `subflows/<namespace>/<name>.yaml`; see [`docs/ai-tasks.md`](docs/ai-tasks.md).
...
```bash
# Run Vitest unit tests (213 tests)
npm test

# Run Playwright E2E browser tests (71 tests, parallel)
