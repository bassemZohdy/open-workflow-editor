...
- Until a provider key is configured, the gateway returns `503` and the sub-flow stubs produce deterministic mock results (`[mock-llm] …`, `[mock-agent] …`) so the demo engine can validate the whole orchestration.
