...
- **Deployment bundle:** `generateDeploymentBundle` scans the workflow (incl. nested containers) for AI delegations via `findAiDelegations`, then ships a runnable `ai/<name>.yaml` per referenced sub-flow (`prompt-llm` / `ai-agent`): copied into the Docker image (`COPY ai/ /app/ai/`), embedded as ConfigMap keys (`ai/<name>.yaml`) with `items`/`subPath` mounts, `WORKFLOW_SUBFLOW_PATH=/app/ai`, and documented in the bundle README. Sub-flow YAMLs are generated from the canonical contracts — replace them before deploying if you customized a sub-flow in the editor.
