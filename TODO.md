...
```bash
npm test             # Vitest unit tests (88)
npm run test:browser # Playwright E2E tests (68, parallel workers)
...
- [x] `npm test` — **88 unit tests pass** (13 in `src/ideParity.test.ts`, +4 breadcrumb +2 reorder, +4 Task 16 AI builders, +5 gateway AI endpoints, +2 demo AI delegation, +4 Task 34 `buildLibraryRows`, +6 Task 35/37 deployment bundle, +4 Task 38 sub-flow execution).
- [x] `npm run test:browser` — **68 Playwright E2E tests pass** (parallel workers; +1 Task 34 regression, +4 Task 35/36/37/38: AI bundle artifacts, rail icon strip, scaffolded user sub-flow bundle, demo sub-flow execution).
