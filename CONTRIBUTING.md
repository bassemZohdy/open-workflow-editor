...
- **`main` is protected** — all changes land through pull requests. (Repo setting: *Settings → Branches → Branch protection rules → `main` → require status checks (CI), require up-to-date branch, and disallow force pushes. This must be enabled in GitHub — it cannot be configured from the repository contents.)
- **`develop` is the integration branch**: work happens on `feat/…`/`fix/…` branches off `develop`, merges into `develop`, and `develop` merges into `main` for releases. Create `develop` from `main` once and add the same protection settings.
...
npm test             # Vitest unit tests (65)
npm run test:browser # Playwright E2E (61, parallel workers)
